...
class PureByteJEPA(nn.Module):
    def __init__(
        ...
        *,
        ...
        if target_source not in {"codebook", "contextual_only"}:
        ...
        if target_source == "contextual_only" and num_target_codebooks != 1:
            raise ValueError("TARGET_SOURCE='contextual_only' currently requires NUM_TARGET_CODEBOOKS=1")
        ...
        if self.contextual_target or self.contextual_aux_weight > 0.0 or self.target_source == "contextual_only":
        ...
        with torch.no_grad():
            if self.target_source == "contextual_only":
                if self.target_encoder is None:
                    raise RuntimeError("TARGET_SOURCE='contextual_only' requires a target encoder")
