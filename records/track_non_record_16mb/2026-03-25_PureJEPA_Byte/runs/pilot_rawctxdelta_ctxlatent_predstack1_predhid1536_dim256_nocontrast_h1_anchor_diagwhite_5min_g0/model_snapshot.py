...
class PureByteJEPA(nn.Module):
    ...
    def training_step(self, byte_ids: Tensor) -> dict[str, Tensor]:
        target_ids, payload_mask, num_bytes, context_latents, pred_target, pred_parallel, pred_magnitude, action_latents, target_latents, teacher_states, loss_sum, future_aux_loss, parallel_loss_sum, magnitude_loss_sum, teacher_orth_loss_sum, aux_loss_sum = (
    ...
    def validation_step(self, byte_ids: Tensor) -> dict[str, Tensor]:
        target_ids, payload_mask, num_bytes, context_latents, pred_target, pred_parallel, pred_magnitude, action_latents, _, _, loss_sum, _, _, _, _, _ = self._compute_latent_metrics(byte_ids)
