...
class PureByteJEPA(nn.Module):
    def __init__(
        ...
        *,
        ...
        if lm_probe_input not in {"pred_target", "pred_residual", "context_plus_pred", "context_plus_pred_parallel", "action", "context_plus_action"}:
    ...
    def _contextual_delta_targets(self, contextual_states: Tensor, *, steps: int = 1) -> tuple[Tensor, Tensor, Tensor]:
    ...
    def _contrastive_infonce_loss(
        ...
        if self.contrastive_mode == "future_state":
            ...
            if self.target_source == "rotation_scale_delta":
                ...
                _, contrastive_targets, _ = self._contextual_delta_targets(teacher_states, steps=self.contrastive_future_steps)
    ...
    ) -> tuple[Tensor, Tensor, Tensor, Tensor | None, Tensor | None, Tensor, Tensor | None, Tensor | None, Tensor | None]:
        ...
        with torch.no_grad():
            if self.target_source in {"contextual_only", "contextual_delta", "rotation_scale_delta", "sphere_delta", "additive_delta"}:
                ...
                if self.target_source == "contextual_only":
                    ...
                    contextual_targets, target_latents, target_parallel = self._contextual_delta_targets(
                        contextual_states,
    ...
    def _lm_probe_logits(
        ...
        *,
        ...
        flat_pred = self._lm_probe_features(pred_target, pred_parallel, context_latents, action_latents)
        if detach_input:
    ...
    def _compute_latent_metrics(
        self, byte_ids: Tensor
    ) -> tuple[Tensor, Tensor, Tensor, Tensor, Tensor, Tensor | None, Tensor | None, Tensor, Tensor, Tensor, Tensor, Tensor]:
        (
            target_ids,
            context_latents,
            pred_target,
    ...
    def training_step(self, byte_ids: Tensor) -> dict[str, Tensor]:
        target_ids, payload_mask, num_bytes, context_latents, pred_target, pred_parallel, action_latents, target_latents, teacher_states, loss_sum, parallel_loss_sum, aux_loss_sum = (
        ...
        contrastive_loss = self._contrastive_infonce_loss(pred_target, pred_parallel, context_latents, teacher_states, payload_mask)
    ...
    def validation_step(self, byte_ids: Tensor) -> dict[str, Tensor]:
        target_ids, payload_mask, num_bytes, context_latents, pred_target, pred_parallel, action_latents, _, _, loss_sum, _, _ = self._compute_latent_metrics(byte_ids)
