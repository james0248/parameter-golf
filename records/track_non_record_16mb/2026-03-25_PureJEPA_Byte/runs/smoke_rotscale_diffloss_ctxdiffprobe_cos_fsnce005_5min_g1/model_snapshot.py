...
class PureByteJEPA(nn.Module):
    def __init__(
        ...
        *,
        ...
        if target_source not in {"codebook", "contextual_only", "contextual_delta", "rotation_scale_delta"}:
        ...
        if self.contextual_target or self.contextual_aux_weight > 0.0 or self.target_source in {"contextual_only", "contextual_delta", "rotation_scale_delta"}:
    ...
    def _predict_target_latents(self, byte_ids: Tensor) -> tuple[Tensor, Tensor, Tensor, Tensor | None, Tensor, Tensor | None, Tensor | None]:
        ...
        with torch.no_grad():
            if self.target_source in {"contextual_only", "contextual_delta", "rotation_scale_delta"}:
