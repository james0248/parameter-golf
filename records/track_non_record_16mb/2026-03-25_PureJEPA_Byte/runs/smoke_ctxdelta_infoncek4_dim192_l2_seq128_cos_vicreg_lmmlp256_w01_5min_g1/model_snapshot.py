...
class PureByteJEPA(nn.Module):
    def __init__(
        ...
        *,
        ...
        if lm_probe_input not in {"pred_target", "pred_residual", "context_plus_pred"}:
    ...
    def _contrastive_infonce_loss(self, pred_target: Tensor, teacher_states: Tensor | None, payload_mask: Tensor) -> Tensor:
        ...
        mask = payload_mask[:, : contrastive_targets.size(1)].reshape(-1)
        if int(mask.to(dtype=torch.int32).sum().item()) <= 1:
            return pred_target.new_zeros(())
        flat_query = query.reshape(-1, query.size(-2) * query.size(-1))[mask]
    ...
    def _predict_target_latents(self, byte_ids: Tensor) -> tuple[Tensor, Tensor, Tensor, Tensor, Tensor | None, Tensor | None]:
        ...
        return target_ids, context_latents, pred_target, target_latents, contextual_targets, teacher_states
    ...
    def _lm_probe_features(self, pred_target: Tensor, context_latents: Tensor) -> Tensor:
        ...
        elif self.lm_probe_input == "pred_residual":
    ...
    def _compute_latent_metrics(
        ...
        target_ids, context_latents, pred_target, target_latents, contextual_targets, teacher_states = self._predict_target_latents(byte_ids)
    ...
    def training_step(self, byte_ids: Tensor) -> dict[str, Tensor]:
        target_ids, payload_mask, num_bytes, context_latents, pred_target, target_latents, teacher_states, loss_sum, aux_loss_sum = (
        ...
        contrastive_loss = self._contrastive_infonce_loss(pred_target, teacher_states, payload_mask)
    ...
    def validation_step(self, byte_ids: Tensor) -> dict[str, Tensor]:
        target_ids, payload_mask, num_bytes, context_latents, pred_target, _, _, loss_sum, _ = self._compute_latent_metrics(byte_ids)
