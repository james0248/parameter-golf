...
class PureByteJEPA(nn.Module):
    ...
    def _contrastive_infonce_loss(
        ...
        if self.contrastive_mode == "future_state":
            ...
            if self.target_source == "contextual_only":
                ...
                query = context_latents[:, : key.size(1)].unsqueeze(2).float() + scaled_delta
            else:
    ...
    def training_step(self, byte_ids: Tensor) -> dict[str, Tensor]:
        target_ids, payload_mask, num_bytes, context_latents, pred_target, pred_parallel, pred_magnitude, action_latents, target_latents, teacher_states, loss_sum, parallel_loss_sum, magnitude_loss_sum, aux_loss_sum = (
    ...
    def validation_step(self, byte_ids: Tensor) -> dict[str, Tensor]:
        target_ids, payload_mask, num_bytes, context_latents, pred_target, pred_parallel, pred_magnitude, action_latents, _, _, loss_sum, _, _, _ = self._compute_latent_metrics(byte_ids)
