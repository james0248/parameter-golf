...
class PureByteJEPA(nn.Module):
    def __init__(
        ...
        *,
        ...
        if target_source not in {"codebook", "contextual_only", "contextual_delta", "rotation_scale_delta", "sphere_delta"}:
            raise ValueError(f"Unknown TARGET_SOURCE={target_source!r}")
        if lm_probe_input not in {"pred_target", "pred_residual", "context_plus_pred", "action", "context_plus_action"}:
        ...
        if target_source in {"contextual_only", "contextual_delta", "rotation_scale_delta"} and num_target_codebooks != 1:
        ...
        if self.contextual_target or self.contextual_aux_weight > 0.0 or self.target_source in {"contextual_only", "contextual_delta", "rotation_scale_delta", "sphere_delta"}:
        ...
        if self.lm_probe_weight > 0.0:
            ...
            elif self.lm_probe_input == "context_plus_pred":
    ...
    def _contextual_delta_targets(self, contextual_states: Tensor, *, steps: int = 1) -> tuple[Tensor, Tensor]:
        ...
        if self.contrastive_mode == "future_state":
            ...
            if self.target_source == "contextual_only":
                ...
                query = _reconstruct_rotation_scale_next_state(
                    context_latents[:, : key.size(1)].unsqueeze(2),
                ...
                query = context_latents[:, : key.size(1)].unsqueeze(2).float() + pred_target[:, : key.size(1)].float()
        else:
            if self.target_source == "rotation_scale_delta":
                ...
                _, contrastive_targets = self._contextual_delta_targets(teacher_states, steps=self.contrastive_future_steps)
    ...
    def _predict_target_latents(self, byte_ids: Tensor) -> tuple[Tensor, Tensor, Tensor, Tensor | None, Tensor, Tensor | None, Tensor | None]:
        ...
        with torch.no_grad():
            if self.target_source in {"contextual_only", "contextual_delta", "rotation_scale_delta", "sphere_delta"}:
                ...
                if self.target_source == "contextual_only":
                    ...
                    contextual_targets, target_latents = self._contextual_delta_targets(
                        contextual_states,
        ...
        return target_ids, context_latents, pred_target, action_latents, target_latents, contextual_targets, teacher_states
    ...
    def _lm_probe_features(self, pred_target: Tensor, context_latents: Tensor, action_latents: Tensor | None) -> Tensor:
    ...
    def _lm_probe_logits(
        self, pred_target: Tensor, context_latents: Tensor, action_latents: Tensor | None, *, detach_input: bool
    ) -> Tensor:
        if self.lm_head is None:
            raise RuntimeError("LM probe requested without lm_head")
        flat_pred = self._lm_probe_features(pred_target, context_latents, action_latents)
        if detach_input:
    ...
    def _lm_probe_nll_sum(
        ...
        *,
        ...
        if self.lm_head is None:
            return pred_target.new_zeros(())
        logits = self._lm_probe_logits(pred_target, context_latents, action_latents, detach_input=detach_input)
    ...
    ) -> tuple[Tensor, Tensor, Tensor, Tensor, Tensor | None, Tensor, Tensor, Tensor, Tensor, Tensor]:
        target_ids, context_latents, pred_target, action_latents, target_latents, contextual_targets, teacher_states = self._predict_target_latents(byte_ids)
    ...
    def training_step(self, byte_ids: Tensor) -> dict[str, Tensor]:
        target_ids, payload_mask, num_bytes, context_latents, pred_target, action_latents, target_latents, teacher_states, loss_sum, aux_loss_sum = (
        ...
        contrastive_loss = self._contrastive_infonce_loss(pred_target, context_latents, teacher_states, payload_mask)
    ...
    def validation_step(self, byte_ids: Tensor) -> dict[str, Tensor]:
        target_ids, payload_mask, num_bytes, context_latents, pred_target, action_latents, _, _, loss_sum, _ = self._compute_latent_metrics(byte_ids)
