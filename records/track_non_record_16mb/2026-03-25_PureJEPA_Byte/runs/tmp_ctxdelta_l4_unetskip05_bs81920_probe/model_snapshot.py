...
class SequenceEncoder(nn.Module):
    def __init__(
        ...
        *,
        causal: bool,
        unet_skip: bool,
        unet_skip_init: float,
    ):
        super().__init__()
        self.causal = causal
        self.tok_emb = nn.Embedding(vocab_size, model_dim)
        self.pos_emb = nn.Parameter(torch.zeros(1, max_seq_len, model_dim))
        self.use_unet_skip = bool(unet_skip)
        self.blocks = nn.ModuleList(
            [EncoderBlock(model_dim, num_heads, mlp_hidden_dim, dropout) for _ in range(num_layers)]
        )
        self.num_encoder_layers = num_layers // 2
        self.num_decoder_layers = num_layers - self.num_encoder_layers
        self.num_skip_weights = min(self.num_encoder_layers, self.num_decoder_layers) if self.use_unet_skip else 0
        if self.num_skip_weights > 0:
            self.skip_weights = nn.Parameter(
                torch.full((self.num_skip_weights, model_dim), float(unet_skip_init), dtype=torch.float32)
            )
        else:
    ...
    def forward(self, token_ids: Tensor) -> Tensor:
        ...
        if self.use_unet_skip and self.num_skip_weights > 0:
            skips: list[Tensor] = []
            ...
            for i in range(self.num_decoder_layers):
                ...
                x = self.blocks[self.num_encoder_layers + i](x, attn_mask)
        else:
...
def _diagonal_whiten(x: Tensor, eps: float) -> Tensor:
    flat = x.float().reshape(-1, x.size(-1))
    mean = flat.mean(dim=0, keepdim=True)
    var = flat.var(dim=0, unbiased=False, keepdim=True)
    whitened = (x.float() - mean) / torch.sqrt(var + eps)
    return whitened.to(dtype=x.dtype)


def _remove_top_pcs(x: Tensor, topk: int) -> Tensor:
    if topk <= 0:
        return x
    with torch.autocast(device_type=x.device.type, enabled=False):
        ...
        k = min(int(topk), eigvecs.size(1))
        if k <= 0:
            return x
        top = eigvecs[:, -k:]
        centered_x = x.float() - mean
        filtered = centered_x - (centered_x @ top) @ top.T
        out = filtered + mean
    return out.to(dtype=x.dtype)


def _project_out_basis(x: Tensor, basis: Tensor, eps: float = 1e-4) -> Tensor:
    with torch.autocast(device_type=x.device.type, enabled=False):
        basis_matrix = basis.float().transpose(-1, -2)
        basis_t = basis_matrix.transpose(-1, -2)
        eye = torch.eye(basis_matrix.size(-1), device=basis_matrix.device, dtype=basis_matrix.dtype)
        gram = basis_t @ basis_matrix
        rhs = basis_t @ x.float().unsqueeze(-1)
        coeff = torch.linalg.solve(gram + eps * eye, rhs)
        proj = (basis_matrix @ coeff).squeeze(-1)
        out = x.float() - proj
    return out.to(dtype=x.dtype)
...
class PureByteJEPA(nn.Module):
    def __init__(
        ...
        *,
        ...
        encoder_unet_skip: bool,
        encoder_unet_skip_init: float,
        target_future_steps: int,
        aux_target_future_steps: str,
        aux_target_weight: float,
        contextual_delta_normalization: str,
        contextual_delta_reference: str,
        contextual_delta_whitening: str,
        contextual_delta_whiten_eps: float,
        contextual_delta_span: int,
        contextual_delta_remove_topk: int,
        contextual_delta_l1_weight: float,
        contextual_state_projector: str,
        ...
        if encoder_unet_skip_init < 0:
            raise ValueError(f"ENCODER_UNET_SKIP_INIT must be non-negative, got {encoder_unet_skip_init}")
        if aux_target_weight < 0:
            raise ValueError(f"AUX_TARGET_WEIGHT must be non-negative, got {aux_target_weight}")
        ...
        if contextual_delta_whitening not in {"none", "diag"}:
            raise ValueError(f"Unknown CONTEXTUAL_DELTA_WHITENING={contextual_delta_whitening!r}")
        if contextual_delta_whiten_eps <= 0:
            raise ValueError(f"CONTEXTUAL_DELTA_WHITEN_EPS must be positive, got {contextual_delta_whiten_eps}")
        if contextual_delta_span <= 0:
            raise ValueError(f"CONTEXTUAL_DELTA_SPAN must be positive, got {contextual_delta_span}")
        if contextual_delta_remove_topk < 0:
            raise ValueError(f"CONTEXTUAL_DELTA_REMOVE_TOPK must be non-negative, got {contextual_delta_remove_topk}")
        if contextual_delta_l1_weight < 0:
            raise ValueError(f"CONTEXTUAL_DELTA_L1_WEIGHT must be non-negative, got {contextual_delta_l1_weight}")
        if contextual_state_projector not in {"none", "linear"}:
            raise ValueError(f"Unknown CONTEXTUAL_STATE_PROJECTOR={contextual_state_projector!r}")
        ...
        self.encoder_unet_skip = bool(encoder_unet_skip)
        self.encoder_unet_skip_init = float(encoder_unet_skip_init)
        self.target_future_steps = int(target_future_steps)
        aux_steps: list[int] = []
        for raw_part in aux_target_future_steps.split(","):
            part = raw_part.strip()
            if not part:
                continue
            step = int(part)
            if step <= 0:
                raise ValueError(f"AUX_TARGET_FUTURE_STEPS entries must be positive, got {step}")
            aux_steps.append(step)
        self.aux_target_future_steps = tuple(aux_steps)
        ...
        self.contextual_delta_whitening = contextual_delta_whitening
        self.contextual_delta_whiten_eps = float(contextual_delta_whiten_eps)
        self.contextual_delta_span = int(contextual_delta_span)
        self.contextual_delta_remove_topk = int(contextual_delta_remove_topk)
        self.contextual_delta_l1_weight = float(contextual_delta_l1_weight)
        self.contextual_state_projector_kind = contextual_state_projector
        ...
        self.online_state_projector: nn.Module | None = None
        self.target_state_projector: nn.Module | None = None
        ...
        self.online_encoder = SequenceEncoder(
            ...
            dropout=dropout,
            causal=True,
            unet_skip=self.encoder_unet_skip,
            unet_skip_init=self.encoder_unet_skip_init,
        )
        ...
        if self.contextual_state_projector_kind == "linear":
            online_proj = nn.Linear(model_dim, model_dim, bias=False)
            with torch.no_grad():
                online_proj.weight.copy_(torch.eye(model_dim, dtype=online_proj.weight.dtype))
            self.online_state_projector = online_proj
            if self.target_encoder_mode == "ema" and needs_target_encoder:
                target_proj = nn.Linear(model_dim, model_dim, bias=False)
                target_proj.load_state_dict(self.online_state_projector.state_dict(), strict=True)
                for param in target_proj.parameters():
                    param.requires_grad_(False)
                self.target_state_projector = target_proj
        ...
        self.future_aux_heads = nn.ModuleDict()
        if self.predictor_style == "direct" and self.aux_target_future_steps:
            output_dim = model_dim * self.num_target_codebooks
            for step in self.aux_target_future_steps:
                self.future_aux_heads[str(step)] = nn.Sequential(
                    nn.LayerNorm(model_dim),
                    nn.Linear(model_dim, predictor_hidden_dim),
                    nn.GELU(),
    ...
    def _contextual_delta_targets(self, contextual_states: Tensor, *, steps: int = 1) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        if self.contextual_delta_whitening == "diag":
            contextual_states = _diagonal_whiten(contextual_states, self.contextual_delta_whiten_eps)
        if self.contextual_delta_remove_topk > 0:
            contextual_states = _remove_top_pcs(contextual_states, self.contextual_delta_remove_topk)
        ...
        if self.contextual_delta_reference == "anchor" and self.contextual_delta_span > 1:
            base_len = contextual_states.size(1) - steps
            index_base = torch.arange(base_len, device=contextual_states.device)
            basis_stack = []
            for offset in range(self.contextual_delta_span):
                gather_idx = (index_base - offset).clamp_min(0)
                basis_stack.append(contextual_states.index_select(1, gather_idx))
            basis = torch.stack(basis_stack, dim=2)
            delta_targets = _project_out_basis(next_targets.squeeze(2), basis).unsqueeze(2)
        else:
    ...
    def _predict_target_latents(
        ...
        if self.online_state_projector is not None:
            context_latents = self.online_state_projector(context_latents)
        ...
        aux_future_preds: dict[int, Tensor] | None = None
        ...
        if self.predictor_style == "action_transition":
            ...
            predictor_features = self.predictor.forward_features(predictor_inputs)
            pred_target = self.predictor.forward_head(predictor_features)
            pred_target = pred_target.view(*predictor_inputs.shape[:-1], self.num_target_codebooks, -1)
            if self.future_aux_heads:
                aux_future_preds = {}
                for step_str, head in self.future_aux_heads.items():
                    aux_pred = head(predictor_features)
                    aux_future_preds[int(step_str)] = aux_pred.view(*predictor_inputs.shape[:-1], self.num_target_codebooks, -1)
    ...
    def _compute_latent_metrics(
        ...
        future_aux_loss = pred_target.new_zeros(())
        if self.aux_target_weight > 0.0 and aux_future_preds:
            if teacher_states is None:
                raise RuntimeError("Auxiliary future targets requested without teacher states")
            aux_terms: list[Tensor] = []
            for step, aux_pred in aux_future_preds.items():
                if self.target_source != "contextual_delta":
                    continue
                _, aux_targets, _, _ = self._contextual_delta_targets(teacher_states, steps=step)
                aux_len = aux_targets.size(1)
                if aux_len <= 0:
                    continue
                aux_pred = aux_pred[:, :aux_len]
                aux_target_ids = target_ids[:, step - 1 : step - 1 + aux_len]
                aux_payload_mask = self._payload_mask(aux_target_ids)
                if int(aux_payload_mask.to(dtype=torch.int32).sum().item()) == 0:
                    continue
                aux_per_pos_loss = self._per_position_loss(aux_pred, aux_targets)
                aux_terms.append(aux_per_pos_loss[aux_payload_mask].mean())
            if aux_terms:
                future_aux_loss = torch.stack(aux_terms).mean()
        sparse_delta_loss = pred_target.new_zeros(())
        if self.contextual_delta_l1_weight > 0.0:
            masked_pred = pred_target.float()[payload_mask]
            if masked_pred.numel() > 0:
                sparse_delta_loss = masked_pred.abs().mean()
        ...
        return (
            ...
            loss_sum,
            future_aux_loss.float(),
            sparse_delta_loss.float(),
    ...
    def training_step(self, byte_ids: Tensor) -> dict[str, Tensor]:
        target_ids, payload_mask, num_bytes, context_latents, pred_target, pred_parallel, pred_magnitude, action_latents, target_latents, teacher_states, loss_sum, future_aux_loss, sparse_delta_loss, parallel_loss_sum, magnitude_loss_sum, teacher_orth_loss_sum, aux_loss_sum = (
        ...
        loss = (
            base_loss
            + self.aux_target_weight * future_aux_loss
            + self.contextual_delta_l1_weight * sparse_delta_loss
        ...
        return {
            ...
            "future_aux_loss": future_aux_loss.detach(),
            "sparse_delta_loss": sparse_delta_loss.detach(),
    ...
    def validation_step(self, byte_ids: Tensor) -> dict[str, Tensor]:
        target_ids, payload_mask, num_bytes, context_latents, pred_target, pred_parallel, pred_magnitude, action_latents, _, _, loss_sum, _, _, _, _, _, _ = self._compute_latent_metrics(byte_ids)
    ...
    def update_target_encoder(self) -> None:
        ...
        if self.target_state_projector is not None and self.online_state_projector is not None and self.target_encoder_mode == "ema":
            for tgt, src in zip(self.target_state_projector.parameters(), self.online_state_projector.parameters(), strict=True):
                tgt.data.mul_(decay).add_(src.data, alpha=1.0 - decay)
...
def configure_optimizers(model: PureByteJEPA, args: Any) -> list[torch.optim.Optimizer]:
    ...
    if optimizer_kind == "muon":
        ...
        control_param_ids = {
            id(param)
            for name, param in main_named_params
            if any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS)
        }
        matrix_params = [
            param
            for name, param in main_named_params
            if param.ndim == 2 and id(param) not in embed_param_ids and id(param) not in control_param_ids
        ]
