...
class PureByteJEPA(nn.Module):
    def __init__(
        ...
        *,
        ...
        if self.contextual_target or self.contextual_aux_weight > 0.0:
    ...
    def _contextual_residual_targets(self, anchor_targets: Tensor, contextual_targets: Tensor) -> Tensor:
        ...
        contextual_repr = F.normalize(contextual_targets.float(), dim=-1, eps=1e-8)
        residual = contextual_repr - (contextual_repr * anchor_repr).sum(dim=-1, keepdim=True) * anchor_repr
    ...
    def _predict_target_latents(self, byte_ids: Tensor) -> tuple[Tensor, Tensor, Tensor, Tensor, Tensor | None]:
        ...
        pred_target = self.predictor(context_latents)
        pred_target = pred_target.view(*context_latents.shape[:-1], self.num_target_codebooks, -1)
    ...
    def _lm_probe_logits(self, pred_target: Tensor, *, detach_input: bool) -> Tensor:
    ...
    def _lm_probe_nll_sum(
        ...
        *,
        ...
        logits = self._lm_probe_logits(pred_target, detach_input=detach_input)
    ...
    def _compute_latent_metrics(self, byte_ids: Tensor) -> tuple[Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor]:
        ...
        return target_ids, payload_mask, num_bytes, pred_target.float(), target_latents.float(), loss_sum, aux_loss_sum.float()

    def training_step(self, byte_ids: Tensor) -> dict[str, Tensor]:
        target_ids, payload_mask, num_bytes, pred_target, target_latents, loss_sum, aux_loss_sum = self._compute_latent_metrics(byte_ids)
        ...
        lm_probe_loss = self._lm_probe_nll_sum(pred_target, target_ids, payload_mask, detach_input=True) / num_bytes
    ...
    def validation_step(self, byte_ids: Tensor) -> dict[str, Tensor]:
        target_ids, payload_mask, num_bytes, pred_target, _, loss_sum, _ = self._compute_latent_metrics(byte_ids)
        if self.lm_head is not None:
            nll_sum_nat = self._lm_probe_nll_sum(pred_target, target_ids, payload_mask, detach_input=False)
        else:
